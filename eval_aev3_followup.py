...
# -*- coding: utf-8 -*-
"""
eval_aev3_followup.py

AE-v3-only longitudinal/follow-up evaluation for FaceMoCap healthy-reference
reconstructions.
...
- followup_visit_global_metrics.csv
    median metrics per participant and acquisition date, across movements.
...
python eval_aev3_followup.py ... --min_dates 3
"""
...
def write_report(
    ...
    lines = []
    lines.append("AE-v3 follow-up evaluation")
    lines.append("==========================")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out_dir", default="align_mean_movement_refactor/aev3_followup_eval")
...
def main() -> None:
    ...
    region_df.to_csv(out_dir / "followup_region_metrics.csv", index=False)
    side_df.to_csv(out_dir / "followup_side_metrics.csv", index=False)
    region_side_df.to_csv(out_dir / "followup_region_side_metrics.csv", index=False)

    visit_movement, visit_global = aggregate_visits(sample_df)
    ...
    # Movement-specific baseline change is useful because not every visit has all movements.
    movement_change_parts = []
    for movement, g in visit_movement.groupby("movement"):
    ...
    if not args.no_figures:
        ...
        plot_region_side_heatmaps(region_side_df, out_dir, metric="trajectory_mae")
        plot_region_side_heatmaps(region_side_df, out_dir, metric="hypokinesia_proxy")
        plot_region_side_heatmaps(region_side_df, out_dir, metric="hyperkinesia_proxy")

    write_report(out_dir, audit_df, sample_df, visit_global, participant_df, min_dates=args.min_dates)
    print(f"[OK] Wrote follow-up evaluation to: {out_dir}")
    print("Main files:")
    for name in [
        "followup_sample_metrics.csv",
        "followup_visit_global_metrics.csv",
